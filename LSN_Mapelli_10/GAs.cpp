...
vector<unsigned int> GAs :: GetPath (){
   return path;
}

vector<unsigned int> GAs :: GetNewPath (){
   return newpath;
}

void GAs :: SetPath(vector<unsigned int> member){
   path = member;
}

void GAs :: SetNewPath(vector<unsigned int> member){
   newpath = member;
}
...
void GAs :: Sorter(){
   ...
   vector <double> l;
   for (unsigned int i = 0; i < m_R ; i++){
      l.push_back(NormL(GetMember(i),0));
   }
   sort(l.begin(), l.end());
   vector <double> L = l;
...
unsigned int GAs :: Selection(){ // selection
   double accu = 0;
   double accu2=0;
   double sum = 0;
...
void GAs :: PairPermutation (){
   // unsigned int n = m_R-1;
   unsigned int i,j;
   member = path;
